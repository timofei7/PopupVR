...
macro EasyLine2:
	case [| EasyLine2 |]:
		yield [|
			splitter2 as GUIStyle = GUIStyle( GUI.skin.button )
			splitter2.stretchWidth = true
			splitter2.border = RectOffset(1, 1, 1, 1)
			splitter2.margin = RectOffset(0, 0, 7, 7)

			EasyRow:
				GUILayout.Box("", splitter, GUILayout.Height(1F))
		|]
	case [| EasyLine2 $pixels |]:
		yield [|
			splitter2 as GUIStyle = GUIStyle( GUI.skin.button )
			splitter2.stretchWidth = true
			splitter2.border = RectOffset(1, 1, 1, 1)
	...
	case [| EasyToggleButton $controlTexture, $textureOn, $textureOff, $toggleVariable, $tooltip |]:
		yield [|
			...
			if GUILayout.Button(guiContent, GUILayout.ExpandWidth(false), GUILayout.MinHeight(16)):
				...
				$toggleVariable = not $toggleVariable
		|]
	case [| EasyToggleButton $controlTexture, $textureOn, $textureOff, $toggleVariable, $tooltip, $opt1 |]:
		yield [|
			if $toggleVariable:
				$controlTexture = $textureOn
			else:
				$controlTexture = $textureOff
			guiContent = GUIContent($controlTexture, $tooltip)
			if GUILayout.Button(guiContent, GUILayout.ExpandWidth(false), GUILayout.MinHeight(16), $opt1):
				if $controlTexture == $textureOff:
					$controlTexture = $textureOn
				else:
					$controlTexture = $textureOff
				$toggleVariable = not $toggleVariable
		|]
	case [| EasyToggleButton $controlTexture, $textureOn, $textureOff, $toggleVariable, $tooltip, $opt1, $opt2 |]:
		yield [|
			if $toggleVariable:
				$controlTexture = $textureOn
			else:
				$controlTexture = $textureOff
			guiContent = GUIContent($controlTexture, $tooltip)
			if GUILayout.Button(guiContent, GUILayout.ExpandWidth(false), GUILayout.MinHeight(16), $opt1, $opt2):
				if $controlTexture == $textureOff:
					$controlTexture = $textureOn
				else:
					$controlTexture = $textureOff
...
macro EasyObjectField:
   ...
   case [| EasyObjectField $label,  $expression, $type, $opt1, $opt2 |]:
	   yield [| $expression = UndoableObjectField($label, $expression, $type, $opt1, $opt2) |]
