...
public class RageButton : MonoBehaviour, IRageButton {
	[SerializeField]private RageButtonData _data;
	public RageButtonData Data {
		get {
			if (_data == null) _data = new RageButtonData();
			return _data;
		}
		set { _data = value; }
	}
	...
	public void ClickCallbacks() {
		foreach (MonoBehaviour script in Data.ToExecuteOnClick)
			script.SendMessage("OnClick", gameObject, SendMessageOptions.DontRequireReceiver);
