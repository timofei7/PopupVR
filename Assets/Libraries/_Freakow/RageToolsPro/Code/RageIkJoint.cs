...
[ExecuteInEditMode]
public class RageIkJoint : MonoBehaviour {

	public bool Live = true;
	public float MinLimiterAngle = 45f;
	public float MaxLimiterAngle = 315f;
	private static int _staticGizmoRadius;
	[SerializeField]private int _gizmoRadius;
	public int GizmoRadius {
		get {
			if (_staticGizmoRadius > 0)
				_gizmoRadius = _staticGizmoRadius;
			return _gizmoRadius;
		}
		set { 
			if (value > 0)
				_staticGizmoRadius = value;
			_gizmoRadius = value;
		}
	}
	...
	public Vector3 RestDirection { 
		get { return transform.rotation * Quaternion.AngleAxis(RestAngle, Vector3.forward) * Vector3.right; }
	}

	public void Awake() {
		_staticGizmoRadius = GizmoRadius;
	}
