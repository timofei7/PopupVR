...
public class RageSpline : MonoBehaviour, IRageSpline {
	...
	public void MaterialCheck() {
		...
		if (PerspectiveMode) {
			...
			if (Mrenderer.sharedMaterials[0] == null && renderer.sharedMaterials[1] == null) AssignMaterials();
		} else {
			...
			switch (GetPhysics()) {
				...
				case Physics.Boxed:
					...
					if (lastPhysicsVertsCount != getSplitsLength || (BoxColliders == null || BoxColliders[0] == null)) {
						...
						int i = 0;
						var splits = GetSplits(GetPhysicsColliderCount(), 0f, 1f);
						var boxColliderDepth = GetBoxColliderDepth();
						var physicsNormalOffset = GetPhysicsNormalOffset();
						var offsetFactor = (boxColliderDepth * 0.5f - physicsNormalOffset);

						foreach (BoxCollider boxCol in BoxColliders) {
							if (boxCol == null) continue;
							if (splinePhysicsMaterial != null) boxCol.material = splinePhysicsMaterial;
							if (PhysicsIsTrigger) boxCol.isTrigger = true;
							int i2 = i + 1;
							...
							pos = splits[i].position - norm * offsetFactor;
							pos2 = splits[i2].position - norm2 * offsetFactor;
							...
							boxCol.gameObject.transform.localScale = new Vector3(GetPhysicsZDepth(), ((pos + norm * boxColliderDepth * 0.5f) - (pos2 + norm2 * boxColliderDepth * 0.5f)).magnitude, 1f * boxColliderDepth);
							i++;
						}
